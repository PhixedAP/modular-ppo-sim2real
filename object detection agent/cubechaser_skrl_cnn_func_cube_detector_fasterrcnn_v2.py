...
class Policy(GaussianMixin, Model):
    ...
    def compute(self, inputs, role):
        ...
        if False and self.compute_counter < 100 and getpass.getuser() == 'phil':
...
env_cfg = load_cfg_from_registry("CCC-skrl-cd-fastercnn-v2", "env_cfg_entry_point")
env = gym.make("CCC-skrl-cd-fastercnn-v2", cfg=env_cfg, asynchronous=False)
...
size_action_history = env.unwrapped.cfg.size_action_history
models = {}
models["policy"] = Policy(env.observation_space, env.action_space, device, clip_actions=True, cube_detector_neurons=cd_neurons, size_action_history=size_action_history)
models["value"] = Value(env.observation_space, env.action_space, device, cube_detector_neurons=cd_neurons, size_action_history=size_action_history)
...
cfg["discount_factor"] = 0.95  # gamma
cfg["lambda"] = 0.95
...
cfg["experiment"]["directory"] = "runs/torch/CCC-skrl-cd-fastercnn-v2"
...
if training:
    ...
    path = "/home/phil/Documents/24-12-08_05-12-47-861514_PPO/agent_69000.pt"  # best ever
    agent.load(path)
    trainer.eval()
env.close()
