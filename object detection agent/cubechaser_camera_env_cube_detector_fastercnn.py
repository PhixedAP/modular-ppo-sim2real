...
@configclass
class CCCEnvCBFRCNNCfg(DirectRLEnvCfg):
    # env

    evaluation_mode = True  # used for creating evaluatin prints, used for thesis

    decimation = 2  # environment steps till new action
    episode_length_s = 10.0 if evaluation_mode else 20.0  # steps: math.ceil(self.max_episode_length_s / (self.cfg.sim.dt * self.cfg.decimation))
    action_scale = 1.0
    num_actions = 2
    ...
    # removed robot history, hard to implement on the robot side. Increased action history instead
    # size_robot_history = 18  # last 3 entries (each entry size 6)
    size_cube_detector_history = 5
    update_cube_detector_history_each_steps = 5
    num_observations = cube_detector_neurons * size_cube_detector_history + size_action_history  #  + size_robot_history  # 1000 for resnet
    num_states = 0
    num_envs = 4  # 32 8  # 12 seems to be max with images on my laptop!, 8 for scenery

    required_distance_to_goal = 0.3  # TODO: 0.15
    ...
    # robot
    model_name = "010_4wis_finetuned_camera.usd"  # "006_4wis_added_clipping_to_camera.usd"
...
class CCCEnvCBFRCNNC(DirectRLEnv):
    cfg: CCCEnvCBFRCNNCfg
    if getpass.getuser() == 'phil':
    ...
    def __init__(self, cfg: CCCEnvCBFRCNNCfg, render_mode: str | None = None, **kwargs):
        ...
        self.penalty_for_too_many_action_changes = torch.ones(self.cfg.num_envs, device=self.sim.device, dtype=torch.float32) * -0.02
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        if self.observation_counter > 1:
            ...
            penalty_action_changes = torch.where(
                ...
                torch.ones(self.cfg.num_envs, device=self.sim.device, dtype=torch.float32) * self.penalty_for_too_many_action_changes,
                torch.zeros(self.cfg.num_envs, device=self.sim.device, dtype=torch.float32))
            if self.observation_counter < 50:
