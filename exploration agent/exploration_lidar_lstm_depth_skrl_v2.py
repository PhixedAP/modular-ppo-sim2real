...
class Policy(GaussianMixin, Model):
    ...
    def compute(self, inputs, role):
        ...
        # print("inputs[states] shape: ", inputs["states"].shape)  # inputs[states] shape:  torch.Size([4, 70681])
        ...
        if self.export_images and (self.compute_counter < 20):  #  or (1500 < self.compute_counter < 2500)
...
env_cfg = load_cfg_from_registry("Exploration-Lidar-v2", "env_cfg_entry_point")
env = gym.make("Exploration-Lidar-v2", cfg=env_cfg, asynchronous=False)  # try order_enforce? Probably not: If to enforce the order of :meth:`gymnasium.Env.reset` before :meth:`gymnasium.Env.step` and :meth:`gymnasium.Env.render` functions
...
cfg["learning_epochs"] = 8  # reduced from 8, presumably more stable
# 16 ? 512 ? 128: num envs
# old: 16 * 512 / 128
# (16 * 512) / env.num_envs
...
cfg["experiment"]["directory"] = "runs/torch/Exploration-Lidar-Lstm-Depth-v2"
...
cfg_trainer = {"timesteps": 300000 if training else 20000, "headless": headless}  # 1000000
...
if training:
    ...
    # "/home/phil/Documents/24-11-21_01-21-35-155916_PPO_RNN/agent_60000.pt", model which uses reflectance patches etc, was not that convincing...
    # "/home/phil/Documents/24-11-06_08-34-59-950245_PPO_RNN/agent_66000.pt"  # best model in simulations but pre reflectance patches
    # "/home/phil/Documents/24-12-12_15-13-11-763961_PPO_RNN/explo_new_try_best_agent.pt"  # always wants to drive left, results in crashes...
    # "/home/phil/Documents/24-12-12_21-57-51-400216_PPO_RNN/agent_146000.pt" ## good in simulation but evaluation not that much,,,
    path = "/home/phil/Documents/24-12-12_21-57-51-400216_PPO_RNN/agent_40000.pt"
    print("starting evaluation for path: ", path)
    agent.load(path)
    trainer.eval()
env.close()
