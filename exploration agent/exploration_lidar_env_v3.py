...
@configclass
class ExplorationLidarEnvCfgV3(DirectRLEnvCfg):
    evaluation_mode = False  # used for creating evaluation prints, used for thesis
    add_patches_and_noise_to_depth_image = True
    ...
    episode_length_factor = 3.0 if evaluation_mode else 2.0
    ...
    update_frequency_depth_image_steps = 5  # Note: value has no effect while depth_image_history_size is 0!

    default_velocity = 10.0  # 10.0
    ...
    maximum_discovery_reward = 2.5  # 25.0  # 50.0  # Note: increased from 10.0  # reward per 10 percentage discovered

    penalty_for_touching_obstacle = -1.0  # -15.0
    ...
    penalty_for_too_many_action_changes_per_step = -0.8 / max_steps_per_episode

    penalty_for_driving_backwards_for_too_long = 0.0  # -1.0/450.0
    driving_backwards_penalty_after_steps = 10

    reward_for_driving_forward_per_step = 1.6 / max_steps_per_episode  # 1.5
    ...
    robot_staying_at_same_position_penalty_per_step = -1.5 / max_steps_per_episode
...
class ExplorationLidarEnvV3(DirectRLEnv):
    cfg: ExplorationLidarEnvCfgV3
    image_path = "/home/phil/university/thesis/data/images/"
    observation_counter = 0
    randomization_counter = 0
    export_grid_maps = False
    export_robot_history = True

    def __init__(self, cfg: ExplorationLidarEnvCfgV3, render_mode: str | None = None, **kwargs):
        ...
        self.mecanum_controller = mecanum_controller.MecanumController(self.cfg.num_envs, self.sim.device, default_velocity=self.cfg.default_velocity)  # , default_velocity=20.0
    ...
    def store_robot_positions_history_env_0_as_png(self):
        if len(self.robot_position_history_env_0) > 0:
            ...
            robot_positions_np = robot_positions_np - self.scene.env_origins[0].tolist()
            plt.figure(figsize=(10, 10))
            plt.scatter(robot_positions_np[0, 1], robot_positions_np[0, 0], marker='x', s=700, linewidth=10,
                        label="Start")
            plt.plot(robot_positions_np[:, 1], robot_positions_np[:, 0], linewidth=3, label="Path")
            plt.xlim(0, 7)
            plt.ylim(7, 0)
            plt.xlabel('X')
            plt.ylabel('Y')
            total_reward_env_0 = self.total_reward_sum[0].item()
            total_time_steps_env_0 = self.current_run_observation_counter_per_env[0].item()
            plt.title('Robot Position History: ' + str(total_reward_env_0) + " - " + str(total_time_steps_env_0))
            plt.grid(True, which='both', color='0.95', linestyle='-', linewidth=0.5)
            plt.tight_layout()
    ...
    def _get_observations(self) -> dict:
        ...
        if self.cfg.add_patches_and_noise_to_depth_image:
            ...
            depth_camera_image = self.add_depth_noise(depth_camera_image, noise_std=0.06, min_depth=0.0,
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        if self.export_grid_maps and 0 in env_ids:
            ...
            self.discovery_percentages_history_env_0 = []
            self.robot_position_history_env_0 = []
        elif self.export_robot_history and 0 in env_ids:
        ...
        if self.cfg.evaluation_mode:
            ...
            if self.total_episode_counter > 0 and self.count_collisions.item() > 0 and (self.total_episode_counter - self.count_collisions.item()) > 0:
                ...
                print(
                    f"temp_obs_counter: {self.temp_obs_counter} - temp_pre_physic_counter: {self.temp_pre_physic_counter} - temp_apply_action_counter: {self.temp_apply_action_counter}")
                print("----------------------------")
        ...
        # z: 0.01
        if False and self.cfg.evaluation_mode:
