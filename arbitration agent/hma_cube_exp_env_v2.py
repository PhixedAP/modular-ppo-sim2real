...
@configclass
class HmaCubeExpEnvV2Cfg(DirectRLEnvCfg):
    evaluation_mode = True
    active_environment = "cube"  # "exploration" or "cube" or "multi"

    state_inputs_version = "value_and_ce_value_and_logprobs_and_ce_logprobs_each_and_ce_policy_ratio"  #value_and_ce_value value_and_ce_value_and_ce_logprobs_min value_and_ce_value_and_logprobs_and_ce_logprobs_each_and_ce_policy_ratio "value_only" value_and_max_policy_std value_and_logprobs  value_logprobs_crosseval
    ...
    environment_dependent_episode_timesteps = 600
    episode_length_factor = 1.5 if evaluation_mode else 1.0

    if active_environment == "exploration":
        environment_dependent_episode_timesteps = 1200
        episode_length_factor = 1.0 if evaluation_mode else 1.5
    elif active_environment == "cube":
        episode_length_factor = 1.5
        environment_dependent_episode_timesteps = 600
    elif active_environment == "multi":
        episode_length_factor = 1.5 if evaluation_mode else 1.0
        environment_dependent_episode_timesteps = 600

    episode_length_s = sim_dt * environment_dependent_episode_timesteps * episode_length_factor * decimation / 2
    max_steps_per_episode = environment_dependent_episode_timesteps * episode_length_factor
    ...
    cube_size_action_history = 8
    cube_size_cube_detector_history = 6
    cube_update_cube_detector_history_each_steps = 2
    ...
    size_log_odds_probabilities = 0
    size_value_ce = 0
    size_agents_rating_action_other_agents = 0
    size_agents_cross_evaluation_log_probs_min = 0
    size_agents_cross_evaluation_log_probs_sum = 0
    size_agents_cross_evaluation_log_probs_mean = 0
    size_agents_cross_evaluation_log_probs_each = 0

    size_policy_ratio_ce = 0

    if state_inputs_version == "value_only":
        size_log_odds_probabilities = 0
        size_agents_rating_action_other_agents = 0
    elif state_inputs_version == "value_and_ce_value":
        size_value_ce = 1
    elif state_inputs_version == "value_and_log_prob_cross_evaluation_min":
        size_log_odds_probabilities = 0
        size_agents_rating_action_other_agents = 0
        size_agents_cross_evaluation_log_probs_min = num_different_agents
    elif state_inputs_version == "value_and_log_prob_cross_evaluation_each":
        size_agents_cross_evaluation_log_probs_each = num_different_agents*num_actions
    elif state_inputs_version == "value_logprobs_crosseval":
    ...
    required_distance_to_goal = 0.35
    update_frequency_ma_observation = 2
    agent_selection_history_size = 5
    ma_observation_history_size = 3
    single_observation_size = (size_value_network + size_value_ce + size_log_odds_probabilities + size_policy_ratio_ce) * num_different_agents + size_agents_rating_action_other_agents + size_agents_cross_evaluation_log_probs_min + size_agents_cross_evaluation_log_probs_sum + size_agents_cross_evaluation_log_probs_mean + size_agents_cross_evaluation_log_probs_each
    num_observations = single_observation_size * ma_observation_history_size + agent_selection_history_size


    penalty_for_touching_obstacle = -3.0
    reward_for_goal_reached = 4.0
    reward_distance_change_factor = 2.5
    maximum_discovery_reward = 0.0  # 2.5
    penalty_per_action_change = -9.0/max_steps_per_episode
    reward_per_step_using_explo_agent = 2.0/max_steps_per_episode
    penalty_per_step = 0.0 / max_steps_per_episode
    only_give_distance_reward_if_cube_agent_active = True

    random_steps_for_value_collection_for_normalization = 0
...
class ExplorationAgent:
    # /home/phil/Documents/24-11-06_08-34-59-950245_PPO_RNN/agent_66000.pt  # old
    #/home/phil/Documents/24-12-12_15-13-11-763961_PPO_RNN/explo_new_try_best_agent.pt  # issue: always turning left... results in crash
    ...
    def __init__(self, cfg: HmaCubeExpEnvV2Cfg, device):
    ...
    def act(self, observations):
        ...
        # log_std = self.agent.policy.log_std_parameter
        # print("log_std_parameter: ", log_std)
        # print("log_std exp: ", torch.exp(log_std))
        # temp_dist = self.agent.policy.distribution()
        # print("distribution: ", temp_dist)
        # print("entropy: ", temp_dist.entropy())
        # print("temp_dist stddev: ", temp_dist.stddev[0])
        # print("log probs: ", policy[1])
        # log_std = torch.clamp(log_std, -20, 2)
        # print("log_std clipped: ", log_std)
        ...
        return mean_actions, policy[1], value[0]
    ...
    def get_log_prob_for_action(self, actions, reduction=None):
        log_prob = self.agent.policy.distribution().log_prob(actions)
        if reduction is None:
            return log_prob
        if reduction == "min":
            return torch.min(log_prob, dim=-1).values.unsqueeze(1)
        if reduction == "sum":
            return log_prob.sum(dim=-1).unsqueeze(1)
        if reduction == "mean":
            return log_prob.mean(dim=-1).unsqueeze(1)
        return log_prob
    ...
    def get_policy(self, observations):
        policy_rnn = {"rnn": self.rnn_states["policy"]}
        return self.agent.policy.act({"states": observations, **policy_rnn}, role="policy")
...
class CubeDetectorAgent:
    # previous: /home/phil/Documents/24-09-24_08-34-18-330494_PPO/agent_66000.pt
    model_path = "/home/phil/Documents/24-12-08_05-12-47-861514_PPO/"  # best ever
    ...
    def __init__(self, cfg: HmaCubeExpEnvV2Cfg, device):
        ...
        self.models["policy"] = CubePolicy(self.observation_space, self.action_space, self.device, clip_actions=True,
                                           ...
                                           size_action_history=cfg.cube_size_action_history)
    ...
    def get_policy(self, observations):
        return self.agent.policy.act({"states": observations}, role="policy")



class HmaCubeExpEnvV2(DirectRLEnv):
    cfg: HmaCubeExpEnvV2Cfg
    image_path = "/home/phil/university/thesis/data/images/"
    observation_counter = 0
    randomization_counter = 0
    export_images = False
    export_grid_maps = False
    observation_logging_env0 = False
    images_saved_counter = 0

    def __init__(self, cfg: HmaCubeExpEnvV2Cfg, render_mode: str | None = None, **kwargs):
        super().__init__(cfg)
        ...
        # evaluation measures
        self.total_episode_counter = -self.cfg.num_envs
        ...
        self.total_module_changes = 0
        self.total_timesteps_explo_agent_active = 0
        self.total_timesteps_cube_agent_active = 0
        ...
        self.observation_logging_for_plotting_env_0 = {"explo_agent_log_probs": [],
                                                       "explo_agent_value": [],
                                                       "cube_agent_log_probs": [],
                                                       "cube_agent_value": [],
                                                       "explo_log_probs_cube_actions": [],
                                                       "explo_log_probs_cube_actions_min": [],
                                                       "explo_log_probs_cube_actions_sum": [],
                                                       "explo_log_probs_cube_actions_mean": [],
                                                       "explo_value_with_cube_action": [],
                                                       "explo_policy_with_cube_action_ratio": [],
                                                       "cube_log_probs_expo_actions": [],
                                                       "cube_log_probs_expo_actions_min": [],
                                                       "cube_log_probs_expo_actions_sum": [],
                                                       "cube_log_probs_expo_actions_mean": [],
                                                       "cube_value_with_explo_action": [],
                                                       "cube_policy_with_explo_action_ratio": [],
                                                       "selected_agent": [],
                                                       "cube_detector_value": [],
                                                       "distance_to_cube": [],
                                                       }
        ...
        self.tracking_min_value_estimates = torch.zeros(self.cfg.num_different_agents, device=self.sim.device,
                                               dtype=torch.float32)
        self.tracking_max_value_estimates = torch.ones(self.cfg.num_different_agents, device=self.sim.device,
                                              dtype=torch.float32)
        self.goal_reached_tracking = torch.zeros(36, device=self.sim.device, dtype=torch.float32)

        if self.cfg.random_steps_for_value_collection_for_normalization > 0:
            self.min_value_estimates = torch.zeros(self.cfg.num_different_agents, device=self.sim.device, dtype=torch.float32)
            self.max_value_estimates = torch.ones(self.cfg.num_different_agents, device=self.sim.device,
                                                   dtype=torch.float32)
        else:
            if self.cfg.evaluation_mode:
                self.min_value_estimates = torch.tensor([-1.2116,  0.0000], device=self.sim.device,
                                                       dtype=torch.float32)
                self.max_value_estimates = torch.tensor([1.0117, 5.4856], device=self.sim.device,
                                                      dtype=torch.float32)
            else:
                self.min_value_estimates = torch.tensor([-1.4380,  0.0000], device=self.sim.device,
                                                       dtype=torch.float32)
                self.max_value_estimates = torch.tensor([1.0, 6.1816], device=self.sim.device,
                                                      dtype=torch.float32)
    ...
    def _setup_scene(self):
        ...
        if self.cfg.active_environment == "multi":  # "exploration" or "cube" or "multi"
            ...
            obstacle_cuboids_positions = [[3.5, 3.0, 1.0, 0.0], [3.25, 1.9, 0.70711, 0.70711], [3.0, 4.25, 1.0, 0.0],
                                          [3.5, 1.125, 0.70711, 0.70711], [4.0, 4.0, 1.0, 0.0], [3.0, 5.8, 1.0, 0.0],
                                          [6.0, 4.0, 1.0, 0.0]]
        ...
        elif self.cfg.active_environment == "exploration":
            column_positions = [[5.75, 1.25], [1.2, 1.7]]
            for index, column_position in enumerate(column_positions):
                self.cfg.obstacle_column_cfg.func(
                    prim_path="/World/envs/env_.*/obstacle_column_{}".format(index),
            ...
            obstacle_cuboids_positions = [[3.5, 3.0, 1.0, 0.0], [1.0, 4.5, 0.70711, 0.70711], [3.0, 4.25, 1.0, 0.0],
                                          [3.5, 1.4, 0.96593, 0.25882], [5.0, 5.0, 0.70711, 0.70711],
                                          [3.0, 5.8, 1.0, 0.0], [6.0, 4.0, 0.70711, 0.70711]]
            for index, obstacle_cuboid_position in enumerate(obstacle_cuboids_positions):
                self.cfg.obstacle_cuboid_cfg.func(
                    prim_path="/World/envs/env_.*/obstacle_cuboid_{}".format(index),
                    cfg=self.cfg.obstacle_cuboid_cfg,
        ...
        elif self.cfg.active_environment == "cube":
    ...
    def _apply_action(self) -> None:
        action_to_apply = self.last_applied_action
        if self.observation_counter < self.cfg.random_steps_for_value_collection_for_normalization:
            action_to_apply = self.exp_last_action
        velocities, positions = self.mecanum_controller.continuous_actions_into_velocities_and_positions(action_to_apply)
    ...
    def store_observation_logging_for_plotting_env_0_as_png(self, reward_sum, found_cube=False):
        fig, axes = plt.subplots(7, 1, figsize=(30, 60))  # Create two subplots
        ...
        for key, value in self.observation_logging_for_plotting_env_0.items():
            ...
            elif key in ["explo_agent_log_probs", "cube_agent_log_probs"]:
                # third plot
                axes[2].plot(self.observation_logging_for_plotting_env_0[key], label=key)
            elif key in ["explo_log_probs_cube_actions", "cube_log_probs_expo_actions"]:
                # fourth plot
                # print("self.observation_logging_for_plotting_env_0: ", self.observation_logging_for_plotting_env_0[key])
                temp_np = np.array(self.observation_logging_for_plotting_env_0[key])
                axes[3].plot(temp_np[:, 0], label=key + "_0")
                axes[3].plot(temp_np[:, 1], label=key + "_1")
            elif key in ["distance_to_cube"]:
                # fifth plot
                axes[6].plot(self.observation_logging_for_plotting_env_0[key], label=key)
            elif key in ["explo_agent_value", "cube_agent_value"]:
                axes[0].plot(value, label=key)
            elif key in ["explo_log_probs_cube_actions_min", "explo_log_probs_cube_actions_sum", "explo_log_probs_cube_actions_mean",
                         "cube_log_probs_expo_actions_min", "cube_log_probs_expo_actions_sum", "cube_log_probs_expo_actions_mean"]:
                axes[4].plot(self.observation_logging_for_plotting_env_0[key], label=key)
            elif key in ["explo_policy_with_cube_action_ratio", "cube_policy_with_explo_action_ratio"]:
                axes[5].plot(self.observation_logging_for_plotting_env_0[key], label=key)
        ...
        axes[5].grid(True, which='both', color='0.95', linestyle='-', linewidth=0.5)
        axes[5].legend(fontsize=20)

        axes[6].set_xlabel('Steps')
        axes[6].set_ylabel('Distance')
        axes[6].set_title('Distance to Cube')
        axes[6].grid(True, which='both', color='0.95', linestyle='-', linewidth=0.5)
        axes[6].legend(fontsize=20)
        ...
        # also store as pickle
        with open(self.image_path + "observation_logging_for_plotting_" + str(self.observation_counter) + ".pkl", "wb") as f:
            pickle.dump(self.observation_logging_for_plotting_env_0, f)
    ...
    def _get_observations(self) -> dict:
        ...
        depth_camera_image -= 0.12  # TODO: try what happens if subtracting some value. Maybe less crashes?
        ...
        if not has_incomplete_lidar_data:
            ...
            exp_action_history_normalized = (self.exp_action_history.clone() + 1.0) / 2.0
            explo_agent_input = torch.cat((explo_agent_input, exp_action_history_normalized), dim=1)
        ...
        explo_agent_log_probs = self.log_odds_to_probability_tensor(explo_agent_log_odds)
        ...
        # Note: Use last applied action to ensure a somewhat stable transition between actions if modules change
        ...
        if (self.cfg.cube_update_cube_detector_history_each_steps > 0 and
                self.current_run_observation_counter_per_env[0] % self.cfg.cube_update_cube_detector_history_each_steps == 0):
        ...
        cube_action_history_normalized = (self.cube_action_history.clone() + 1.0) / 2.0
        cube_obs = torch.cat((self.cube_detector_history, cube_action_history_normalized), dim=1)

        cube_agent_action, cube_agent_log_odds, cube_agent_value = self.cube_agent.act(cube_obs)
        cube_agent_log_probs = self.log_odds_to_probability_tensor(cube_agent_log_odds)
        ...
        # apply log prob from other agent
        explo_log_probs_cube_actions = self.exploration_agent.get_log_prob_for_action(cube_agent_action)
        explo_log_probs_cube_actions = self.log_odds_to_probability_tensor(explo_log_probs_cube_actions)

        explo_log_probs_cube_actions_min = self.log_odds_to_probability_tensor(torch.min(explo_log_probs_cube_actions, dim=-1).values.unsqueeze(1))
        explo_log_probs_cube_actions_sum = self.log_odds_to_probability_tensor(torch.sum(explo_log_probs_cube_actions, dim=-1).unsqueeze(1))
        explo_log_probs_cube_actions_mean = self.log_odds_to_probability_tensor(torch.mean(explo_log_probs_cube_actions, dim=-1).unsqueeze(1))
        ...
        _, explo_policy_with_cube_action_ratio, _ = self.exploration_agent.get_policy(explo_agent_input)
        explo_policy_with_cube_action_ratio = explo_policy_with_cube_action_ratio - cube_agent_log_odds
        explo_policy_with_cube_action_ratio = self.log_odds_to_probability_tensor(explo_policy_with_cube_action_ratio)


        cube_log_probs_expo_actions = self.cube_agent.get_log_prob_for_action(explo_agent_action)
        cube_log_probs_expo_actions = self.log_odds_to_probability_tensor(cube_log_probs_expo_actions)

        cube_log_probs_expo_actions_min = self.log_odds_to_probability_tensor(torch.min(cube_log_probs_expo_actions, dim=-1).values.unsqueeze(1))
        cube_log_probs_expo_actions_sum = self.log_odds_to_probability_tensor(torch.sum(cube_log_probs_expo_actions, dim=-1).unsqueeze(1))
        cube_log_probs_expo_actions_mean = self.log_odds_to_probability_tensor(torch.mean(cube_log_probs_expo_actions, dim=-1).unsqueeze(1))
        ...
        _, cube_policy_with_explo_action_ratio, _ = self.cube_agent.get_policy(cube_obs)
        cube_policy_with_explo_action_ratio = cube_policy_with_explo_action_ratio - explo_agent_log_odds
        cube_policy_with_explo_action_ratio = self.log_odds_to_probability_tensor(cube_policy_with_explo_action_ratio)

        '''
        Multi agent
        '''

        # collect min and max values for normalization

        min_explo_value = torch.min(explo_agent_value)
        max_explo_value = torch.max(explo_agent_value)
        min_cube_value = torch.min(cube_agent_value)
        max_cube_value = torch.max(cube_agent_value)

        self.tracking_min_value_estimates = torch.min(torch.tensor([min_explo_value, min_cube_value], device=self.sim.device),
                                                 self.tracking_min_value_estimates)
        self.tracking_max_value_estimates = torch.max(torch.tensor([max_explo_value, max_cube_value], device=self.sim.device),
                                                 self.tracking_max_value_estimates)


        if self.observation_counter < self.cfg.random_steps_for_value_collection_for_normalization or self.observation_counter < self.cfg.update_value_normalization_min_max_till_timestep:
            self.min_value_estimates = torch.min(torch.tensor([min_explo_value, min_cube_value], device=self.sim.device),
                                                 self.min_value_estimates)
            self.max_value_estimates = torch.max(torch.tensor([max_explo_value, max_cube_value], device=self.sim.device),
                                                 self.max_value_estimates)

        # normalize
        if self.cfg.random_steps_for_value_collection_for_normalization >= 0:
            explo_agent_value = (explo_agent_value - self.min_value_estimates[0]) / (
                        self.max_value_estimates[0] - self.min_value_estimates[0])
            cube_agent_value = (cube_agent_value - self.min_value_estimates[1]) / (
                        self.max_value_estimates[1] - self.min_value_estimates[1])



        if self.observation_logging_env0:
            self.observation_logging_for_plotting_env_0["explo_agent_log_probs"].append(explo_agent_log_probs[0].item())
            self.observation_logging_for_plotting_env_0["explo_agent_value"].append(explo_agent_value[0].item())
            self.observation_logging_for_plotting_env_0["cube_agent_log_probs"].append(cube_agent_log_probs[0].item())
            self.observation_logging_for_plotting_env_0["cube_agent_value"].append(cube_agent_value[0].item())
            explo_log_probs_cube_actions_list = explo_log_probs_cube_actions[0].tolist()
            self.observation_logging_for_plotting_env_0["explo_log_probs_cube_actions"].append(explo_log_probs_cube_actions_list)
            self.observation_logging_for_plotting_env_0["explo_log_probs_cube_actions_min"].append(explo_log_probs_cube_actions_min[0].item())
            self.observation_logging_for_plotting_env_0["explo_log_probs_cube_actions_sum"].append(explo_log_probs_cube_actions_sum[0].item())
            self.observation_logging_for_plotting_env_0["explo_log_probs_cube_actions_mean"].append(explo_log_probs_cube_actions_mean[0].item())
            self.observation_logging_for_plotting_env_0["explo_value_with_cube_action"].append(explo_value_with_cube_action[0].item())
            self.observation_logging_for_plotting_env_0["explo_policy_with_cube_action_ratio"].append(explo_policy_with_cube_action_ratio[0].item())
            cube_log_probs_expo_actions_list = cube_log_probs_expo_actions[0].tolist()
            self.observation_logging_for_plotting_env_0["cube_log_probs_expo_actions"].append(cube_log_probs_expo_actions_list)
            self.observation_logging_for_plotting_env_0["cube_log_probs_expo_actions_min"].append(cube_log_probs_expo_actions_min[0].item())
            self.observation_logging_for_plotting_env_0["cube_log_probs_expo_actions_sum"].append(cube_log_probs_expo_actions_sum[0].item())
            self.observation_logging_for_plotting_env_0["cube_log_probs_expo_actions_mean"].append(cube_log_probs_expo_actions_mean[0].item())
            self.observation_logging_for_plotting_env_0["cube_value_with_explo_action"].append(cube_value_with_explo_action[0].item())
            self.observation_logging_for_plotting_env_0["cube_policy_with_explo_action_ratio"].append(cube_policy_with_explo_action_ratio[0].item())
        ...
        if self.cfg.size_agents_rating_action_other_agents > 0:
            multi_agent_observation = torch.cat((multi_agent_observation, explo_log_probs_cube_actions, cube_log_probs_expo_actions,
                                                 explo_value_with_cube_action, cube_value_with_explo_action), dim=1)

        if self.cfg.size_value_ce > 0:
            multi_agent_observation = torch.cat((multi_agent_observation, explo_value_with_cube_action, cube_value_with_explo_action), dim=1)

        if self.cfg.size_agents_cross_evaluation_log_probs_min > 0:
            multi_agent_observation = torch.cat((multi_agent_observation, explo_log_probs_cube_actions_min, cube_log_probs_expo_actions_min), dim=1)

        if self.cfg.size_agents_cross_evaluation_log_probs_sum > 0:
            multi_agent_observation = torch.cat((multi_agent_observation, explo_log_probs_cube_actions_sum, cube_log_probs_expo_actions_sum), dim=1)

        if self.cfg.size_agents_cross_evaluation_log_probs_mean > 0:
            multi_agent_observation = torch.cat((multi_agent_observation, explo_log_probs_cube_actions_mean, cube_log_probs_expo_actions_mean), dim=1)

        if self.cfg.size_agents_cross_evaluation_log_probs_each > 0:
            multi_agent_observation = torch.cat((multi_agent_observation, explo_log_probs_cube_actions, cube_log_probs_expo_actions), dim=1)

        if self.cfg.size_policy_ratio_ce > 0:
            multi_agent_observation = torch.cat((multi_agent_observation, explo_policy_with_cube_action_ratio, cube_policy_with_explo_action_ratio), dim=1)



        if self.cfg.size_log_odds_probabilities > 0:
            multi_agent_observation = torch.cat((multi_agent_observation, explo_agent_log_probs, cube_agent_log_probs), dim=1)
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        self.distance_to_goal = torch.linalg.norm(self._goal.data.root_pos_w - front_bumper_positions, dim=1,
                                             dtype=torch.float32)
        ...
        self.force_matrix_penalty = torch.where((
                                                            active_sensors_chassis | active_sensors_flw | active_sensors_frw | active_sensors_rlw | active_sensors_rrw | active_sensors_fb) & (
        ...
        self.total_module_changes += torch.sum(self.agent_selection_history[:, -1] != self.agent_selection_history[:, -2]).item()
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        if self.observation_logging_env0 and 0 in env_ids and self.observation_counter > 10:
        ...
        self.goal_reached_tracking = torch.roll(self.goal_reached_tracking, shifts=-num_resets, dims=0)
        self.goal_reached_tracking[-num_resets:] = temp
        ...
        rolling_goal_reached_tracking = torch.sum(self.goal_reached_tracking) / self.goal_reached_tracking.shape[0]
        ...
        print(f"rolling_goal_reached_tracking: {rolling_goal_reached_tracking*100:.2f}% - rounded_total_discovery_reward: {rounded_total_discovery_reward} - count_selected_which_agent [exp, cube]: {count_selected_which_agent_as_list} - total_penalty_for_too_many_action_changes: {rounded_total_penalty_for_too_many_action_changes}")
        print(f"tracking_min_value_estimates: {self.tracking_min_value_estimates} - tracking_max_value_estimates: {self.tracking_max_value_estimates} - min_value_estimates: {self.min_value_estimates} - max_value_estimates: {self.max_value_estimates}")

        if self.reset_counter > 0:
            print(f"tgrc: {self.total_goal_reached_counter} - task success rate: {self.total_goal_reached_counter / self.reset_counter*100:.2f}%")

        if self.cfg.evaluation_mode:
            self.total_episode_counter += num_resets
            self.max_timesteps = max(self.max_timesteps, torch.max(self.current_run_observation_counter_per_env[env_ids]).item())
            ...
            self.total_discovery_percentage_when_not_colliding += torch.sum(
                self.previous_discovery_percentages[env_ids] * (self.total_force_matrix_penalty[env_ids] >= 0.0))
            self.summed_timesteps_when_collision_happened += torch.sum(
                self.current_run_observation_counter_per_env[env_ids] * (
                            self.total_force_matrix_penalty[env_ids] < 0.0))
            self.max_discovery_percentage = max(self.max_discovery_percentage,
                                                torch.max(self.previous_discovery_percentages[env_ids]).item())
            ...
            self.total_timesteps_explo_agent_active += torch.sum(self.count_selected_which_agent[env_ids][:, 0])
            self.total_timesteps_cube_agent_active += torch.sum(self.count_selected_which_agent[env_ids][:, 1])

            self.total_timesteps_if_goal_reached += torch.sum(self.current_run_observation_counter_per_env[env_ids] * (self.previous_distance_to_goal[env_ids] < self.cfg.required_distance_to_goal))
            ...
            if self.total_episode_counter > 0 and self.count_collisions.item() > 0 and (self.total_episode_counter - self.count_collisions.item()) > 0:
                ...
                print(f"success rate no collision: {(self.total_episode_counter - self.count_collisions.item()) / self.total_episode_counter*100:.2f}% - average timesteps: {self.total_timesteps.item() / self.total_episode_counter} - average discovery percentage: {self.total_discovery_percentage.item() / self.total_episode_counter} - average discovery percentage when not colliding: {self.total_discovery_percentage_when_not_colliding.item() / (self.total_episode_counter - self.count_collisions.item()) } - average timesteps when collision happened: {self.summed_timesteps_when_collision_happened.item() / self.count_collisions.item()}")
                print(f"success percentage reaching cube: {self.total_task_successes / self.total_episode_counter * 100:.2f}% - summed_up_initial_distance_to_goal: {self.summed_up_initial_distance_to_goal.item()} - total_task_successes: {self.total_task_successes} - total_module_changes: {self.total_module_changes} - total_timesteps_explo_agent_active: {self.total_timesteps_explo_agent_active} - total_timesteps_cube_agent_active: {self.total_timesteps_cube_agent_active}")
                print(
                    f"average_initial_distance_to_goal: {self.summed_up_initial_distance_to_goal.item() / self.total_episode_counter:.2f}")
                print(f"average_timesteps_per_episode: {self.total_timesteps.item() / self.total_episode_counter:.2f}")
                print(
                    f"average_timesteps_per_episode_if_goal_reached: {self.total_timesteps_if_goal_reached.item() / self.total_task_successes:.2f} - max_timesteps: {self.max_timesteps}")
                print(
                    f"tracking_min_value_estimates: {self.tracking_min_value_estimates} - tracking_max_value_estimates: {self.tracking_max_value_estimates}")
                print("----------------------------")
        ...
        if self.cfg.active_environment == "multi":  # "exploration" or "cube" or "multi"
            ...
            # no cube
            goal_position = self.scene.env_origins[env_ids] + torch.tensor([0.0, 0.0, -0.1], device=self.sim.device)
            default_root_state[:, :3] = goal_position
            self._goal.write_root_pose_to_sim(default_root_state[:, :7], env_ids)
        elif self.cfg.active_environment == "cube":
            ...
            r_tensor = torch.rand((num_resets, 1), device=self.sim.device) * torch.tensor(
                [1.0 + 0.4], device=self.sim.device) + 0.8
            z_tensor = torch.full((num_resets, 1), 0.02, device=self.sim.device)

            goal_pos_tensor = torch.cat(
                (torch.sin(alpha_tensor) * r_tensor, torch.cos(alpha_tensor) * r_tensor, z_tensor),
                dim=1)
            temp_list = torch.tensor([3.5, 3.5, 0], device=self.sim.device)
            goal_new_root_state = goal_pos_tensor + torch.stack([temp_list] * num_resets) + self.scene.env_origins[
                env_ids]
        ...
        self.initial_distance_to_goal[env_ids] = self.distance_to_goal[env_ids].clone()
